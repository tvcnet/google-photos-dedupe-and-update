    ...
    <div class="main">
      ...
      <div class="action-bar">
        ...
        <div class="action-buttons">
          ...
          <button type="button" id="aiDescribe" title="Generate AI descriptions for photos using Gemini">AI Describe</button>
        </div>
...
/* ================================================================
   ...
   ================================================================ */
:root {
    /* Accent — TVCNet sky-500 palette */
    --accent: #0ea5e9;
    --accent-hover: #38bdf8;
    --accent-muted: rgba(14, 165, 233, 0.15);
    --accent-glow: rgba(14, 165, 233, 0.25);
    /* Surfaces — deep dark with glass overtones */
    --bg-base: #070c14;
    --bg-raised: #0c1420;
    --bg-overlay: #101a28;
    ...
    /* Glass surfaces */
    --glass-sidebar: rgba(12, 20, 32, 0.72);
    --glass-header: rgba(14, 165, 233, 0.06);
    /* Borders */
    ...
    --warning-text: #fbbf24;
    /* Overlay backdrop */
    --overlay-filter: blur(16px) brightness(0.25) saturate(0.7);
    /* Tokens */
    --radius-xs: 4px;
    --radius-sm: 6px;
    --radius-md: 8px;
...
#gptk {
    ...
    border-radius: var(--radius-xl); color-scheme: dark;
    background: linear-gradient(160deg, #0c1a2e 0%, #070c14 60%);
    ...
    * { box-sizing: border-box; }
    ...
    .header {
        ...
        background: linear-gradient(135deg, rgba(14, 165, 233, 0.10) 0%, rgba(14, 165, 233, 0.02) 60%);
        border-bottom: 1px solid var(--border-accent);
        gap: 12px; flex-shrink: 0;
        backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px);
        .header-info { display: flex; align-items: center; gap: 10px; flex-shrink: 0; }
        .header-icon { color: var(--accent); display: flex; align-items: center; filter: drop-shadow(0 0 6px rgba(14,165,233,0.5)); }
    ...
    .refresh-albums { cursor: pointer; fill: var(--text-secondary); background: var(--bg-surface); border-radius: var(--radius-sm); padding: 3px; border: 1px solid var(--border-default); transition: all var(--duration-fast) var(--ease); display: flex; justify-content: center; align-items: center; }
    ...
    .main {
        ...
        .button-container {
            background: var(--bg-raised); display: flex; gap: 4px; padding: 5px 10px;
            border-bottom: 1px solid var(--border-subtle);
            #stopProcess { display: none; background: var(--danger); border-color: transparent; color: #fff; }
            #stopProcess:hover { background: var(--danger-hover); }
        }
    }
...
`);
    ...
    */
    ...
    class ApiUtils {
         ...
         * sends it to Gemini 2.0 Flash, and writes the result to Google Photos.
         *
         * @param mediaItems - Single-element array (required by executeWithConcurrency).
         * @returns [true] if description was set, [false] if skipped.
         */
        async aiDescribeOneItem(mediaItems) {
            try {
                ...
                if (itemInfoExt.descriptionFull) {
                    return [false]; // Skip: already described
                }
                ...
                const imageUrl = `${item.thumb}=w1024-h1024`;
                // Call Gemini Vision
         ...
         * at a time to respect Gemini API rate limits.
         *
         * @param mediaItems - Array of media items to process.
         */
        async aiDescribeItems(mediaItems) {
            log(`AI Describe: Processing up to ${mediaItems.length} items`);
    ...
    function updateUI() {
        ...
        function updateActionButtonStates() {
            ...
            setDisabled('aiDescribe', isActiveTab('trash'));
        }
    ...
    function insertUi() {
        ...
        if (document.getElementById('gptk-button')) return;
        ...
        // ── 2. Identify Anchors (broadened for all languages/regions) ──
        const anchors = [
            'button[aria-label*="Create"]',               // "Create" button variants
            'button[aria-label*="create"]',
            'button[aria-label*="Add photos"]',           // Album view
            'button[aria-label*="add photos"]',
            'button[aria-label*="Upload"]',               // Upload button variants
            'button[aria-label*="upload"]',
            'header button[aria-label*="Help"]',          // Help in header
            'header button[aria-label*="Settings"]',      // Settings in header
            '[aria-label*="Support"]',                    // Support button
            '[aria-label="Google Account"]',              // Account avatar
            '[aria-label*="Account"]',                    // Account info area
            'header [data-ogsr-up]',                      // Google header action area
            '.gb_Td',                                     // Google new header class
            '.c9yG5b',                                    // Legacy fallback
            'header > div:last-child > div:last-child'    // Generic header right area
        ];
        ...
        // ── 3. Inject the button ──
        if (target) {
            target.insertAdjacentElement('beforebegin', btnElement);
            console.log('GPTK: ✅ Toolbar button injected successfully');
        } else {
            // FALLBACK: Floating Action Button (FAB) — always visible safety net
            console.warn('GPTK: ⚠️ No toolbar anchor found — deploying Floating Action Button');
            ...
            btnElement.style.transition = 'transform 0.2s cubic-bezier(0.34,1.56,0.64,1), box-shadow 0.2s ease';
            btnElement.style.cursor = 'pointer';
            // Override icon color for FAB contrast
            ...
            btnElement.addEventListener('mouseenter', function() {
                ...
                this.style.boxShadow = '0 8px 32px rgba(14,165,233,0.55), 0 4px 12px rgba(0,0,0,0.4)';
            });
            btnElement.addEventListener('mouseleave', function() {
                this.style.transform = 'scale(1)';
                this.style.boxShadow = '0 6px 24px rgba(14,165,233,0.45), 0 2px 8px rgba(0,0,0,0.3)';
            });
            document.body.appendChild(btnElement);
        }

        // ── 4. Inject Main Window using a TrustedTypes policy ──
        // Both innerHTML AND DOMParser.parseFromString require TrustedHTML on this page.
        // The correct fix is to create a named TrustedTypes policy (extensions in MAIN
        // world are permitted to do this) and wrap our HTML string with it.
        if (!document.getElementById('gptk')) {
            try {
                const preparedHtml = htmlTemplatePrep(gptkMainTemplate);

                // Build a Trusted Types policy if the API is available
                let htmlToInject = preparedHtml;
                if (typeof trustedTypes !== 'undefined' && trustedTypes.createPolicy) {
                    try {
                        const gptkPolicy = trustedTypes.createPolicy('gptk#html', {
                            createHTML: (s) => s
                        });
                        htmlToInject = gptkPolicy.createHTML(preparedHtml);
                    } catch (policyErr) {
                        // Policy name may already exist (e.g. 2nd injection) — try alternate name
                        try {
                            const gptkPolicy2 = trustedTypes.createPolicy('gptk#html2', {
                                createHTML: (s) => s
                            });
                            htmlToInject = gptkPolicy2.createHTML(preparedHtml);
                        } catch (e2) {
                            console.warn('GPTK: Could not create TrustedTypes policy:', e2);
                        }
                    }
                }

                // Inject via a detached container — use firstElementChild (not firstChild)
                // to skip whitespace text nodes, which crash insertAdjacentElement.
                const container = document.createElement('div');
                container.innerHTML = htmlToInject;
                while (container.firstElementChild) {
                    document.body.insertAdjacentElement('afterbegin', container.firstElementChild);
                }
                console.log('GPTK: ✅ Main panel injected successfully');
            } catch (e) {
                console.error('GPTK: Main template injection failed:', e);
            }

            // Styles and listeners run regardless of whether template injection threw —
            // so the click handler is always attached even on partial failure.
    ...
    function initDurableUi() {
        ...
        // Durable loop: re-inject on SPA navigation (e.g. library → album → library).
        // Google Photos is a SPA — navigating between pages removes the toolbar
        // contents and re-renders them, which removes our injected button.
        ...
        setInterval(() => {
            ...
            if (urlChanged) {
                lastUrl = currentUrl;
                console.log('GPTK: URL changed — re-injecting UI');
                // Remove stale button so insertUi() doesn't early-exit on the id check
                document.getElementById('gptk-button')?.remove();
                // Small delay to let the new page's toolbar render
                setTimeout(insertUi, 800);
            } else if (!document.getElementById('gptk-button')) {
                // Button disappeared without a URL change (Google re-rendered toolbar)
                insertUi();
            }
        }, 1000);
    }
    ...
    function getFromStorage(key) {
        ...
        if (key === 'apiSettings' && extSettings && extSettings.geminiApiKey) {
            return extSettings;
        }
    ...
    function initUI() {
        // Inject the UI first — this must succeed even if subsequent setup throws.
        initDurableUi();
