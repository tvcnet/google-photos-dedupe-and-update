...
Plugin URI: https://hackrepair.com/about/hackrepair-smart-embed
...
class HackRepair_Smart_Embed {
    ...
    public static function register_cpt() {
        $labels = array(
            'name'               => _x( 'Smart Embeds', 'post type general name', 'hackrpair-smart-embed' ),
            'singular_name'      => _x( 'Smart Embed', 'post type singular name', 'hackrpair-smart-embed' ),
            'add_new'            => _x( 'Add New', 'Smart Embed', 'hackrpair-smart-embed' ),
            'add_new_item'       => __( 'Add New Smart Embed', 'hackrpair-smart-embed' ),
            'edit_item'          => __( 'Edit Smart Embed', 'hackrpair-smart-embed' ),
            'new_item'           => __( 'New Smart Embed', 'hackrpair-smart-embed' ),
            'view_item'          => __( 'View Smart Embed', 'hackrpair-smart-embed' ),
            'search_items'       => __( 'Search Smart Embeds', 'hackrpair-smart-embed' ),
            'not_found'          => __( 'No Smart Embeds found', 'hackrpair-smart-embed' ),
            'not_found_in_trash' => __( 'No Smart Embeds found in Trash', 'hackrpair-smart-embed' ),
            'parent_item_colon'  => '',
            'menu_name'          => __( 'Smart Embeds', 'hackrpair-smart-embed' ),
        );
        ...
        $args = array(
            ...
            // Hide CPT from sidebar menu; access via Settings tab or direct URL
            'show_in_menu'        => false,
    ...
    public static function add_shortcode_column( $columns ) {
        $columns['smart_embed_shortcode'] = __( 'Smart Embed Shortcode', 'hackrpair-smart-embed' );
        return $columns;
    }
    ...
    public static function add_shortcode_column_content( $column, $post_id ) {
        ...
        echo '<button type="button" class="button hrgse-copy" data-clip="' . esc_attr( $sc_id ) . '">' . esc_html__( 'Copy', 'hackrpair-smart-embed' ) . '</button>';
        echo '<br />';
        echo '<code>' . esc_html( $sc_slug ) . '</code> ';
        echo '<button type="button" class="button hrgse-copy" data-clip="' . esc_attr( $sc_slug ) . '">' . esc_html__( 'Copy', 'hackrpair-smart-embed' ) . '</button>';
        echo '</div>';
    }
    ...
    public static function shortcode( $attrs, $content = null ) {
        ...
        // If URL embeds are enabled and a valid script URL is present, enqueue it.
        if ( self::url_embeds_enabled() && self::is_url_allowed( $script_url ) ) {
    ...
    public static function register_meta_box() {
        add_meta_box(
            'hrgse_settings',
            __( 'Smart Embed Settings', 'hackrpair-smart-embed' ),
    ...
    public static function render_meta_box( $post ) {
        ...
        echo '<p><strong>' . esc_html__( 'Shortcode', 'hackrpair-smart-embed' ) . '</strong><br />';
        echo '<code>' . esc_html( $sc_id ) . '</code> ';
        echo '<button type="button" class="button button-small hrgse-copy" data-clip="' . esc_attr( $sc_id ) . '">' . esc_html__( 'Copy', 'hackrpair-smart-embed' ) . '</button>';
        if ( $sc_slug ) {
            echo '<br /><code>' . esc_html( $sc_slug ) . '</code> ';
            echo '<button type="button" class="button button-small hrgse-copy" data-clip="' . esc_attr( $sc_slug ) . '">' . esc_html__( 'Copy', 'hackrpair-smart-embed' ) . '</button>';
        }
        echo '</p>';
        ...
        ?>
        <p>
            <label for="hrgse_editor_toggle"><strong><?php esc_html_e( 'Editor Mode', 'hackrpair-smart-embed' ); ?></strong></label><br />
            ...
            <span class="description"><?php esc_html_e( 'Enable Visual Mode', 'hackrpair-smart-embed' ); ?></span>
        </p>
        <p>
            <label>
                <input type="checkbox" name="hrgse_responsive" value="1" <?php checked( '1', $responsive ); ?> />
                <?php esc_html_e( 'Responsive wrapper', 'hackrpair-smart-embed' ); ?>
            </label>
        </p>
        <p>
            <label for="hrgse_max_width"><strong><?php esc_html_e( 'Max Width (px)', 'hackrpair-smart-embed' ); ?></strong></label><br />
        ...
        <p>
            <label for="hrgse_wrapper_class"><strong><?php esc_html_e( 'Wrapper CSS class', 'hackrpair-smart-embed' ); ?></strong></label><br />
        ...
        <hr />
        <p>
            <label for="hrgse_script_url"><strong><?php esc_html_e( 'External Script URL (optional)', 'hackrpair-smart-embed' ); ?></strong></label><br />
            ...
            <span class="description"><?php esc_html_e( 'Must Be Enabled in Settings: The URL will load with defer, use HTTPS only, and be restricted to approved domains.', 'hackrpair-smart-embed' ); ?></span>
        </p>
        <p>
            <label><input type="checkbox" name="hrgse_canvas_enable" value="1" <?php checked( '1', $canvas_enable ); ?> /> <strong><?php esc_html_e( 'Insert canvas placeholder', 'hackrpair-smart-embed' ); ?></strong></label><br />
            <label for="hrgse_canvas_id"><?php esc_html_e( 'Canvas ID', 'hackrpair-smart-embed' ); ?></label>
            ...
            <span class="description"><?php esc_html_e( 'Tip: For canvas, leave Responsive wrapper off or set a fixed height in CSS.', 'hackrpair-smart-embed' ); ?></span><br />
            <label for="hrgse_canvas_width"><?php esc_html_e( 'Canvas width (px)', 'hackrpair-smart-embed' ); ?></label>
            ...
            <label for="hrgse_canvas_height"><?php esc_html_e( 'Canvas height (px)', 'hackrpair-smart-embed' ); ?></label>
            ...
            <br />
            <label><input type="checkbox" name="hrgse_canvas_fullwidth" value="1" <?php checked( '1', $canvas_fullwidth ); ?> /> <?php esc_html_e( 'For full width (sets CSS width:100%)', 'hackrpair-smart-embed' ); ?></label>
        </p>
    ...
    public static function enqueue_assets() {
        wp_register_style( 'hrgse-smart-embed', plugins_url( 'css/smart-embed.css', __FILE__ ), array(), '2.4.2' );
    ...
    public static function admin_assets( $hook ) {
        ...
        $is_settings_page      = ( $screen && 'settings_page_hackrpair-smart-embed' === $screen->id );
        $tab_param             = isset( $_GET['tab'] ) ? sanitize_key( wp_unslash( $_GET['tab'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        $is_settings_list_tab  = ( $is_settings_page && 'list' === $tab_param );
        ...
        if ( $is_list_screen || $is_settings_list_tab ) {
            wp_register_script( 'hrgse-admin', plugins_url( 'js/admin.js', __FILE__ ), array( 'jquery' ), '2.4.2', true );
            wp_enqueue_script( 'hrgse-admin' );
            wp_localize_script( 'hrgse-admin', 'hrgse_i18n', array(
                'copied' => __( 'Copied!', 'hackrpair-smart-embed' ),
                'copy'   => __( 'Copy', 'hackrpair-smart-embed' ),
            ) );
        }

        // Settings page examples toggle
        if ( $is_settings_page ) {
            wp_register_script( 'hrgse-settings', plugins_url( 'js/settings.js', __FILE__ ), array( 'jquery' ), '2.4.2', true );
            wp_enqueue_script( 'hrgse-settings' );
            wp_localize_script( 'hrgse-settings', 'hrgse_settings_i18n', array(
                'show' => __( 'Show examples', 'hackrpair-smart-embed' ),
                'hide' => __( 'Hide examples', 'hackrpair-smart-embed' ),
            ) );
        }

        // Editor toggle JS/CSS for Smart Embed edit screens
        if ( $is_editor_screen ) {
            wp_register_style( 'hrgse-admin-css', plugins_url( 'css/admin.css', __FILE__ ), array(), '2.4.2' );
            ...
            wp_register_script( 'hrgse-editor', plugins_url( 'js/editor-toggle.js', __FILE__ ), array( 'wp-data', 'wp-edit-post', 'jquery' ), '2.4.2', true );
            wp_enqueue_script( 'hrgse-editor' );
            wp_localize_script( 'hrgse-editor', 'hrgse_editor_i18n', array(
                'mode_visual'      => __( 'Visual', 'hackrpair-smart-embed' ),
                'mode_code'        => __( 'Code', 'hackrpair-smart-embed' ),
                'switch_to_code'   => __( 'Switch to Code Editor', 'hackrpair-smart-embed' ),
                'switch_to_visual' => __( 'Switch to Visual Editor', 'hackrpair-smart-embed' ),
    ...
    public static function plugin_row_meta( $links, $file ) {
        ...
        $meta_links = array(
            '<a href="https://hackrepair.com/about/hackrepair-smart-embed" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Docs', 'hackrpair-smart-embed' ) . '</a>',
            '<a href="https://hackrepair.com/contact/" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Support', 'hackrpair-smart-embed' ) . '</a>',
            '<a href="https://hackrepair.com/donations/buy-jim-a-coffee" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Donate', 'hackrpair-smart-embed' ) . '</a>',
        );
    ...
    public static function plugin_action_links( $links ) {
        ...
        $settings_link = '<a href="' . esc_url( admin_url( 'options-general.php?page=hackrpair-smart-embed' ) ) . '">' . esc_html__( 'Settings', 'hackrpair-smart-embed' ) . '</a>';
    ...
    public static function admin_menu() {
        ...
        add_options_page(
            __( "The Hack Repair Guy's Smart Embed: Settings", 'hackrpair-smart-embed' ),
            __( 'Smart Embeds', 'hackrpair-smart-embed' ),
            'manage_options',
            'hackrpair-smart-embed',
    ...
    public static function settings_page() {
        if ( ! current_user_can( 'manage_options' ) ) {
            wp_die( esc_html__( 'You do not have sufficient permissions to access this page.', 'hackrpair-smart-embed' ) );
        }
        $current_tab = isset( $_GET['tab'] ) ? sanitize_key( wp_unslash( $_GET['tab'] ) ) : 'settings';
        $base_url    = admin_url( 'options-general.php?page=hackrpair-smart-embed' );
        echo '<div class="wrap">';
        echo '<h1>' . esc_html__( "The Hack Repair Guy's Smart Embed: Settings", 'hackrpair-smart-embed' ) . '</h1>';
        echo '<h2 class="nav-tab-wrapper">';
        printf(
            '<a href="%s" class="nav-tab %s">%s</a>',
            esc_url( $base_url ),
            ( 'settings' === $current_tab ? 'nav-tab-active' : '' ),
            esc_html__( 'Settings', 'hackrpair-smart-embed' )
        );
        printf(
            '<a href="%s" class="nav-tab %s">%s</a>',
            esc_url( add_query_arg( 'tab', 'list', $base_url ) ),
            ( 'list' === $current_tab ? 'nav-tab-active' : '' ),
            esc_html__( 'Smart Embeds', 'hackrpair-smart-embed' )
        );
        printf(
            '<a href="%s" class="nav-tab %s">%s</a>',
            esc_url( add_query_arg( 'tab', 'notes', $base_url ) ),
            ( 'notes' === $current_tab ? 'nav-tab-active' : '' ),
            esc_html__( 'Author Notes', 'hackrpair-smart-embed' )
        );
        echo '</h2>';
        ...
        if ( 'notes' === $current_tab ) {
            ...
            if ( file_exists( $notes ) ) {
                ...
                echo '<p>' . esc_html__( 'Notes file not found.', 'hackrpair-smart-embed' ) . '</p>';
            }
            echo '</div>';
        } elseif ( 'list' === $current_tab ) {
            // Render the Smart Embeds posts list inline to keep tabs consistent.
            ...
            // Set context for the posts list table to pick up the correct post type.
            if ( function_exists( 'set_current_screen' ) ) {
                set_current_screen( 'edit-smart-embed' );
            }
            global $typenow, $post_type;
            $typenow   = 'smart-embed';
            $post_type = 'smart-embed';
            $_REQUEST['post_type'] = 'smart-embed'; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $_GET['post_type']     = 'smart-embed'; // phpcs:ignore WordPress.Security.NonceVerification.Recommended

            $list_table = new WP_Posts_List_Table( array( 'screen' => 'edit-smart-embed' ) );
            $list_table->prepare_items();

            echo '<div class="hrgse-inline-list">';
            echo '<h1 class="wp-heading-inline">' . esc_html__( 'Smart Embeds', 'hackrpair-smart-embed' ) . '</h1> ';
            echo '<a href="' . esc_url( admin_url( 'post-new.php?post_type=smart-embed' ) ) . '" class="page-title-action">' . esc_html__( 'Add New Smart Embed', 'hackrpair-smart-embed' ) . '</a>';
            echo '<hr class="wp-header-end">';
            echo '<form id="posts-filter" method="get">';
            echo '<input type="hidden" name="page" value="hackrpair-smart-embed" />';
            echo '<input type="hidden" name="tab" value="list" />';
            echo '<input type="hidden" name="post_type" value="smart-embed" />';
            $list_table->views();
            $list_table->display();
            echo '</form>';
            echo '</div>';
            ...
            echo '<p>' . esc_html__( 'Smart Embed lets you create a reusable chunk of HTML (like a video iframe, form, or widget) and drop it anywhere using a shortcode.', 'hackrpair-smart-embed' ) . '</p>';

            echo '<h3>' . esc_html__( 'Shortcodes', 'hackrpair-smart-embed' ) . '</h3>';
            $sc1 = '[smart_embed id="123"]';
            $sc2 = '[smart_embed slug="my-embed"]';
            echo '<p><code>' . esc_html( $sc1 ) . '</code> &nbsp; <code>' . esc_html( $sc2 ) . '</code></p>';

            echo '<h3>' . esc_html__( 'Optional attributes', 'hackrpair-smart-embed' ) . '</h3>';
            ...
            echo '<p class="description">' . esc_html__( 'You can set these options in the Smart Embed editor (sidebar). If you also include them in a shortcode, the shortcode values override the saved settings for that instance only.', 'hackrpair-smart-embed' ) . '</p>';

            // Examples toggle
            echo '<p><a href="#" id="hrgse-examples-toggle" class="button-link">' . esc_html__( 'Show examples', 'hackrpair-smart-embed' ) . '</a></p>';
            echo '<div id="hrgse-examples" style="display:none; margin-top:10px;">';
            echo '<h3>' . esc_html__( 'Examples', 'hackrpair-smart-embed' ) . '</h3>';
            echo '<table class="widefat striped" style="margin-top:10px;">';
            echo '<thead><tr><th>' . esc_html__( 'Use case', 'hackrpair-smart-embed' ) . '</th><th>' . esc_html__( 'Shortcode', 'hackrpair-smart-embed' ) . '</th><th>' . esc_html__( 'Notes', 'hackrpair-smart-embed' ) . '</th></tr></thead><tbody>';
            echo '<tr><td>' . esc_html__( 'YouTube/Vimeo video', 'hackrpair-smart-embed' ) . '</td><td><code>[smart_embed slug="product-video" responsive="true" max_width="800"]</code></td><td>' . esc_html__( 'Paste the iframe into the embed. Responsive keeps aspect ratio; set a max width.', 'hackrpair-smart-embed' ) . '</td></tr>';
            echo '<tr><td>' . esc_html__( 'Contact form (third‑party)', 'hackrpair-smart-embed' ) . '</td><td><code>[smart_embed slug="contact-form" class="form-embed"]</code></td><td>' . esc_html__( 'Forms are usually responsive already. Add a class for styling.', 'hackrpair-smart-embed' ) . '</td></tr>';
            echo '<tr><td>' . esc_html__( 'Google Map', 'hackrpair-smart-embed' ) . '</td><td><code>[smart_embed slug="store-map" responsive="true" max_width="600"]</code></td><td>' . esc_html__( 'Maps often benefit from a max width; adjust if too tall.', 'hackrpair-smart-embed' ) . '</td></tr>';
            echo '<tr><td>' . esc_html__( 'Newsletter signup', 'hackrpair-smart-embed' ) . '</td><td><code>[smart_embed slug="newsletter" class="sidebar-embed"]</code></td><td>' . esc_html__( 'Use a wrapper class to control spacing in sidebars.', 'hackrpair-smart-embed' ) . '</td></tr>';
            echo '</tbody></table>';
            echo '</div>';
            ...
            if ( false ) {
            echo '<h3>' . esc_html__( 'Real‑world examples', 'hackrpair-smart-embed' ) . '</h3>';
            echo '<table class="widefat striped" style="margin-top:10px;">';
            echo '<thead><tr>';
            echo '<th>' . esc_html__( 'Use case', 'hackrpair-smart-embed' ) . '</th>';
            echo '<th>' . esc_html__( 'Shortcode', 'hackrpair-smart-embed' ) . '</th>';
            echo '<th>' . esc_html__( 'Notes', 'hackrpair-smart-embed' ) . '</th>';
            echo '</tr></thead><tbody>';

            // Example 1: YouTube
            echo '<tr>';
            echo '<td>' . esc_html__( 'YouTube/Vimeo video', 'hackrpair-smart-embed' ) . '</td>';
            $ex1 = '[smart_embed slug="product-video" responsive="true" max_width="800"]';
            echo '<td><code>' . esc_html( $ex1 ) . '</code> <button type="button" class="button hrgse-copy" data-clip="' . esc_attr( $ex1 ) . '">' . esc_html__( 'Copy', 'hackrpair-smart-embed' ) . '</button></td>';
            echo '<td>' . esc_html__( 'Paste the iframe from the provider into the Smart Embed content. Responsive keeps the aspect ratio; set a sensible max width.', 'hackrpair-smart-embed' ) . '</td>';
            echo '</tr>';

            // Example 2: Contact form
            echo '<tr>';
            echo '<td>' . esc_html__( 'Contact form (third‑party)', 'hackrpair-smart-embed' ) . '</td>';
            $ex2 = '[smart_embed slug="contact-form" class="form-embed"]';
            echo '<td><code>' . esc_html( $ex2 ) . '</code> <button type="button" class="button hrgse-copy" data-clip="' . esc_attr( $ex2 ) . '">' . esc_html__( 'Copy', 'hackrpair-smart-embed' ) . '</button></td>';
            echo '<td>' . esc_html__( 'Most forms are already responsive. Skip the responsive wrapper; add a class for styling.', 'hackrpair-smart-embed' ) . '</td>';
            echo '</tr>';

            // Example 3: Google Map
            echo '<tr>';
            echo '<td>' . esc_html__( 'Google Map', 'hackrpair-smart-embed' ) . '</td>';
            $ex3 = '[smart_embed slug="store-map" responsive="true" max_width="600"]';
            echo '<td><code>' . esc_html( $ex3 ) . '</code> <button type="button" class="button hrgse-copy" data-clip="' . esc_attr( $ex3 ) . '">' . esc_html__( 'Copy', 'hackrpair-smart-embed' ) . '</button></td>';
            echo '<td>' . esc_html__( 'Maps often benefit from a max width. Test the aspect ratio; adjust CSS if it looks too tall.', 'hackrpair-smart-embed' ) . '</td>';
            echo '</tr>';

            // Example 4: Newsletter widget
            echo '<tr>';
            echo '<td>' . esc_html__( 'Newsletter signup', 'hackrpair-smart-embed' ) . '</td>';
            $ex4 = '[smart_embed slug="newsletter" class="sidebar-embed"]';
            echo '<td><code>' . esc_html( $ex4 ) . '</code> <button type="button" class="button hrgse-copy" data-clip="' . esc_attr( $ex4 ) . '">' . esc_html__( 'Copy', 'hackrpair-smart-embed' ) . '</button></td>';
            echo '<td>' . esc_html__( 'Use a wrapper class to control spacing in sidebars or narrow columns.', 'hackrpair-smart-embed' ) . '</td>';
            echo '</tr>';

            // Example 5: Trust badge / ad script
            echo '<tr>';
            echo '<td>' . esc_html__( 'Trust badge / ad script', 'hackrpair-smart-embed' ) . '</td>';
            $ex5 = '[smart_embed slug="trust-badge"]';
            echo '<td><code>' . esc_html( $ex5 ) . '</code> <button type="button" class="button hrgse-copy" data-clip="' . esc_attr( $ex5 ) . '">' . esc_html__( 'Copy', 'hackrpair-smart-embed' ) . '</button></td>';
            echo '<td>' . esc_html__( 'Don’t wrap scripts responsively; let the provider7s script handle sizing. Keep it asynchronous if possible.', 'hackrpair-smart-embed' ) . '</td>';
            echo '</tr>';

            // Example 6: Two‑column layout helper
            echo '<tr>';
            echo '<td>' . esc_html__( 'Float/right‑aligned embed', 'hackrpair-smart-embed' ) . '</td>';
            $ex6 = '[smart_embed slug="badge" class="alignright small-embed"]';
            echo '<td><code>' . esc_html( $ex6 ) . '</code> <button type="button" class="button hrgse-copy" data-clip="' . esc_attr( $ex6 ) . '">' . esc_html__( 'Copy', 'hackrpair-smart-embed' ) . '</button></td>';
            echo '<td>' . esc_html__( 'Create small badges or widgets that sit beside text. Add CSS for .small-embed as needed.', 'hackrpair-smart-embed' ) . '</td>';
            echo '</tr>';

            echo '</tbody></table>';
            }

            echo '<h3 style="margin-top:20px;">' . esc_html__( 'Tips', 'hackrpair-smart-embed' ) . '</h3>';
            echo '<ul class="ul-disc">';
            echo '<li>' . esc_html__( 'Toggle the editor quickly: Shift+Cmd+M (Mac) or Shift+Ctrl+M (Windows).', 'hackrpair-smart-embed' ) . '</li>';
            echo '<li>' . esc_html__( 'If an embed looks squashed, turn off the responsive wrapper or adjust the max width.', 'hackrpair-smart-embed' ) . '</li>';
            echo '<li>' . esc_html__( "If a provider's code injects inline CSS that conflicts with your theme, add a wrapper class and override styles in your theme CSS.", 'hackrpair-smart-embed' ) . '</li>';
            echo '</ul>';
            // Data/options form (below Tips)
            echo '<hr />';
            echo '<form method="post" action="options.php">';
            settings_fields( 'hrgse_settings_group' );
            echo '<h3>' . esc_html__( 'Smart Embed Options', 'hackrpair-smart-embed' ) . '</h3>';

            // URL embeds toggle and allowed domains
            $url_enable = get_option( 'hrgse_enable_url_embeds', 0 );
            echo '<p>';
            echo '<label><input type="checkbox" name="hrgse_enable_url_embeds" value="1" ' . checked( '1', (string) $url_enable, false ) . ' /> ' . esc_html__( 'Enable URL embeds', 'hackrpair-smart-embed' ) . '</label>';
            echo '<br /><span class="description">' . esc_html__( 'Allows enqueuing an external JavaScript via URL from the Smart Embed editor. HTTPS only; restricted to approved domains.', 'hackrpair-smart-embed' ) . '</span>';
            echo '</p>';
            ...
            echo '<p>';
            echo '<label for="hrgse_allowed_domains"><strong>' . esc_html__( 'Allowed domains', 'hackrpair-smart-embed' ) . '</strong></label><br />';
            ...
            echo '<br /><span class="description">' . esc_html__( 'One domain per line. Subdomains of a domain are permitted. Example: cdn.example.com', 'hackrpair-smart-embed' ) . '</span>';
            echo '</p>';

            // Example block
            echo '<div class="notice-info" style="padding:10px;border:1px solid #ccd0d4;background:#f6fbff;">';
            echo '<p><strong>' . esc_html__( 'Example', 'hackrpair-smart-embed' ) . '</strong></p>';
            ...
            echo '<p style="margin:6px 0;">' . esc_html__( 'Allowed domains:', 'hackrpair-smart-embed' ) . ' <code>' . esc_html( $example_host ) . '</code></p>';
            echo '<p style="margin:6px 0;">' . esc_html__( 'In the Smart Embed editor, set External Script URL to:', 'hackrpair-smart-embed' ) . ' <code>https://' . esc_html( $example_host ) . '/widget.js</code></p>';
            echo '<p style="margin:6px 0;" class="description">' . esc_html__( 'The script will load once per page with defer (HTTPS only).', 'hackrpair-smart-embed' ) . '</p>';
            echo '</div>';

            $delete_flag = get_option( 'hrgse_delete_on_deactivate', 0 );
            echo '<p>';
            echo '<label><input type="checkbox" name="hrgse_delete_on_deactivate" value="1" ' . checked( '1', (string) $delete_flag, false ) . ' /> ' . esc_html__( 'Delete Smart Embed tables and data on deactivation', 'hackrpair-smart-embed' ) . '</label>';
            echo '</p>';
            submit_button( __( 'Save Changes', 'hackrpair-smart-embed' ) );
            echo '</form>';
            echo '</div>';
        }
